...
import { GoogleCampaignBuilder } from "@/components/campaign-composer/builders/GoogleCampaignBuilder";
...
function DraftEditor() {
  ...
  const loadAccount = useCallback(async (p: CampaignComposerDraftPayload) => {
    ...
    } else if (p.channel.provider === "google") {
      const { data } = await (supabase as any)
        .from("google_ads_connections")
        .select("id,email,customer_accounts,selected_customer_id,login_customer_id")
        .eq("user_id", u.user.id)
        .maybeSingle();
      const accts = mapList(data?.customer_accounts, ["id"], ["descriptiveName", "name"]);
      setAccount({
        connected: !!data,
        name: data?.email ?? undefined,
        adAccounts: accts.length
          ? accts
          : data?.selected_customer_id
            ? [{ id: String(data.selected_customer_id), name: String(data.selected_customer_id) }]
            : [],
        pixels: [],
      });
      return mergeIntegrationDefaults(p, {
        googleConnectionId: data?.id,
        googleLoginCustomerId: data?.login_customer_id ?? undefined,
        adAccountId: data?.selected_customer_id ?? accts[0]?.id,
      });
    }
    return p;
  }, []);
  ...
  const load = useCallback(async () => {
    ...
    if (!p.channel.googleConnectionId && sync.google_connection_id) p.channel.googleConnectionId = sync.google_connection_id;
      ...
      await fnSave({
        data: {
          ...
          syncPatch: {
            ...
            google_connection_id: p.channel.googleConnectionId,
          },
        },
      });
    }
    ...
    async (intent: "draft_only" | "go_live") => {
      ...
      if (r.dryRun) {
        toast.error("Publikacja zablokowana (tryb testowy)", {
          description:
            "Serwer ma CAMPAIGN_COMPOSER_DRY_RUN=true. Ustaw false w zmiennych środowiskowych serwera, aby wysłać kampanię na konto reklamowe.",
        });
        return;
      }
      ...
      const providerLabel =
        payload.channel.provider === "tiktok"
          ...
          : payload.channel.provider === "linkedin"
            ? "LinkedIn Ads"
            : payload.channel.provider === "google"
              ? "Google Ads"
              : "Meta Ads";
    ...
    <div className="min-h-[calc(100vh-3.5rem)] bg-background">
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 md:px-6">
        ...
        <div className="mt-6 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          ...
          {payload.channel.provider === "google" && <GoogleCampaignBuilder {...builderProps} />}
        </div>
