...
const MAX_IMAGES = 4;
/** ~350 KB w base64 — limit localStorage */
const MAX_DATA_URL_LENGTH = 480_000;

function BrandVisualPage() {
  const { products, active, select, update } = useProducts();
  ...
  const saveRules = () => {
    ...
    update(active.id, { brandVisualRules: draftRules });
    lastSavedRef.current = draftRules;
    toast.success("Zapisano reguły wizualne.");
  };
  ...
  const handleStartOver = () => {
    if (!active) return;
    if (!window.confirm("Wyczyścić reguły i obrazy referencyjne dla tego produktu?")) return;
    setDraftRules("");
  ...
  const handleAddImages = async (files: FileList | null) => {
    ...
    if (next.length > current.length) {
      const patch: { brandVisualImages: string[]; thumbnail?: string } = { brandVisualImages: next };
      if (!active.thumbnail && next[0]) patch.thumbnail = next[0];
      update(active.id, patch);
      toast.success("Dodano obrazy referencyjne.");
    }
  ...
  if (!products.length) {
    return (
      <div className="px-6 md:px-10 py-10 max-w-3xl">
        <AppBackLink to="/products/library" label="Wróć do produktów" className="mb-6" />
        <h1 className="text-2xl font-semibold tracking-tight">Tożsamość wizualna marki</h1>
        <p className="mt-2 text-sm text-muted-foreground">Najpierw dodaj produkt w bibliotece.</p>
        <Link
          to="/products/library"
    ...
    <div className="px-6 md:px-10 py-10 max-w-4xl">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl md:text-3xl font-bold tracking-tight text-foreground">Tożsamość wizualna marki</h1>
          <p className="mt-2 text-sm text-muted-foreground max-w-xl leading-relaxed">
            Wzorce wizualne, które agent stosuje przy kreacjach i opisach grafik — spójnie z Twoim produktem.
          </p>
        </div>
      ...
      {images.length > 0 && (
        <div className="mt-8">
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Obrazy referencyjne</p>
          <div className="mt-3 flex flex-wrap gap-3">
            {images.map((src, i) => (
              <div key={`${i}-${src.slice(0, 40)}`} className="relative group w-24 h-24 rounded-xl overflow-hidden border border-border bg-muted">
          ...
          <p className="mt-2 text-[11px] text-muted-foreground">
            Agent traktuje je jako inspirację stylu (światło, kompozycja). Maks. {MAX_IMAGES} pliki, lokalnie w tej przeglądarce.
          </p>
        </div>
      )}

      <div className="mt-10 rounded-2xl border border-border bg-stone-100/80 dark:bg-muted/40 p-5 md:p-6">
        <h2 className="text-lg font-bold text-foreground">Uniwersalne reguły wizualne</h2>
        <p className="mt-1 text-sm text-muted-foreground">Wzorce obowiązujące we wszystkich kreacjach tego produktu.</p>
        ...
        <p className="mt-3 text-[11px] text-muted-foreground">
          Zapis: automatycznie po kliknięciu poza pole lub użyj przycisku — treść trafia do kontekstu agenta i promptów generacji obrazów.
        </p>
        <button
          type="button"
          onClick={() => {
