...
import { Download, Heart, MessageSquareText, Megaphone, Pencil, SquarePen, ThumbsDown, Trash2 } from "lucide-react";
...
function GalleryPage() {
  ...
  const assignToCampaign = async (it: Item) => {
    const name = window.prompt("Przypisz kreację do kampanii (wpisz nazwę):", it.campaign_name ?? "");
    if (name === null) return;
    const next = name.trim() || null;
    const { error } = await supabase.from("generated_images").update({ campaign_name: next }).eq("id", it.id);
    if (error) {
      toast.error(error.message);
      return;
    }
    setItems((prev) => prev.map((x) => (x.id === it.id ? { ...x, campaign_name: next } : x)));
    toast.success(next ? `Przypisano do kampanii: ${next}` : "Usunięto przypisanie do kampanii");
  };
            ...
            >
              ...
              <div className="p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-1.5 pt-1">
                  ...
                  <button
                    type="button"
                    onClick={() => void assignToCampaign(it)}
                    className="inline-flex items-center justify-center gap-1 rounded-lg border border-border px-2 py-1.5 text-xs font-medium hover:bg-muted"
                  >
                    <Megaphone className="h-3.5 w-3.5" /> Przypisz
                  </button>
                </div>
              </div>
            </div>
