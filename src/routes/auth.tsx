...
function AuthPage() {
  ...
  const handleEmailSubmit = async (e: FormEvent) => {
    ...
    if (missingSupabase) {
      toast.error(
        "Brak konfiguracji Supabase (VITE_SUPABASE_URL / VITE_SUPABASE_PUBLISHABLE_KEY). Dodaj zmienne w Lovable lub w pliku .env i zrestartuj serwer.",
      );
      return;
    }
    setLoading(true);
          ...
          {missingSupabase && (
            <div className="mb-6 rounded-lg border border-destructive/40 bg-destructive/10 px-4 py-3 text-left text-sm text-destructive">
              ...
              <p className="mt-1 text-destructive/90 leading-relaxed">
                ...
                <code className="rounded bg-background/80 px-1">VITE_SUPABASE_PUBLISHABLE_KEY</code>. W Lovable:
                Settings → Environment variables (albo lokalnie plik <code className="rounded bg-background/80 px-1">.env</code>), potem
...
function translateAuthError(message: string): string {
  ...
  if (lower.includes("google_not_configured"))
    return "Logowanie Google na localhost: dodaj do .env jedną linię VITE_LOVABLE_APP_URL=https://twoj-projekt.lovable.app (adres z paska podglądu Lovable), zrestartuj npm run dev. Sekrety z Lovable nie trzeba kopiować — idą przez broker OAuth.";
  if (lower.includes("missing oauth secret") || lower.includes("unsupported provider"))
    return "Google nie ma Client Secret w Supabase (Authentication → Providers → Google). Albo skopiuj klucze z Lovable do lokalnego .env — to szybsze niż konfiguracja w Supabase Dashboard.";
  if (lower.includes("google_auth_failed") || lower.includes("redirect_uri_mismatch"))
    return "Błąd logowania Google. Na localhost: skopiuj klucze z Lovable do .env i dodaj redirect URI http://localhost:8080/api/public/auth/google/callback w Google Cloud Console.";
