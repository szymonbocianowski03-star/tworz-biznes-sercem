...
import { Users, Package, ArrowRight, Palette } from "lucide-react";
import { motion } from "framer-motion";

export const Route = createFileRoute("/products/choose")({
  head: () => ({ meta: [{ title: "Panel produktów — MarketingNow" }] }),
  ...
  return (
    <div className="px-6 md:px-10 py-10 max-w-5xl">
      <div className="max-w-2xl">
        ...
        <h1 className="mt-2 text-3xl md:text-4xl font-semibold tracking-tight">Produkty czy Zespół?</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Wybierz, gdzie chcesz wejść. Ten ekran jest specjalnie prosty, żeby szybko ogarniać strukturę projektu.
        </p>
      </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        ...
        >
          <div className="h-11 w-11 rounded-2xl bg-foreground/5 text-foreground flex items-center justify-center">
            <Package className="h-5 w-5" />
          </div>
          <h2 className="mt-4 text-xl font-semibold tracking-tight">Produkty</h2>
          <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
            Biblioteka produktów, dodawanie i wybór aktywnego produktu do pracy z agentem.
          </p>
          <Link
            to="/products/library"
          ...
          >
            Wejdź w produkty <ArrowRight className="h-4 w-4" />
          </Link>
        </motion.div>
        ...
        >
          ...
          <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
            Użytkownicy, role i dostęp do produktów współdzielonych w zespole.
          </p>
          <Link
            to="/products/team"
