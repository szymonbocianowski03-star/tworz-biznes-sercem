...
import { GoogleAdsIntegrationCard } from "@/components/GoogleAdsIntegrationCard";
...
function IntegrationsPage() {
  useEffect(() => {
    ...
    if (gads === "connected") toast.success(`Połączono Google Ads${name ? ` (${name})` : ""}`);
    else if (gads === "error") toast.error(`Google Ads: ${friendlyGoogleOAuthError(err)}`, { duration: 8000 });
    ...
    if (meta || linkedin || tiktok || gmail || gcal || gads || outlook || outcal) {
