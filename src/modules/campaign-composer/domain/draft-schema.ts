...
export const googleCampaignTypeSchema = z.enum([
  "SEARCH",
  "PERFORMANCE_MAX",
  "DISPLAY",
  "VIDEO",
...
export const campaignComposerDraftPayloadSchema = z.object({
  ...
  channel: z.object({
    ...
    googleLoginCustomerId: z.string().optional(),
  ...
  google: z
    .object({
      ...
      headline: z.string().max(30).default(""),
      description: z.string().max(90).default(""),
      headlines: z.array(z.string().max(30)).default([]),
      descriptions: z.array(z.string().max(90)).default([]),
      keywords: z.array(z.string().max(80)).default([]),
      geoTargets: z.array(z.string()).default(["PL"]),
      youtubeVideoIds: z.array(z.string().max(32)).default([]),
      /** Shopping — Google Merchant Center ID */
      merchantCenterId: z.string().max(32).optional(),
      /** App — ID pakietu / App Store ID */
      appId: z.string().max(120).optional(),
      appStore: z.enum(["GOOGLE_APP_STORE", "APPLE_APP_STORE"]).optional(),
      longHeadlines: z.array(z.string().max(90)).default([]),
      businessName: z.string().max(25).optional(),
      acceptedAtOwnRisk: z.boolean().optional(),
    })
    .optional(),
  structure: z.object({
  ...
  if (partial.provider === "google") {
    return campaignComposerDraftPayloadSchema.parse({
      ...
      google: {
        ...
        headlines: ["Oferta specjalna", "Sprawdź teraz", "Zamów online"],
        descriptions: ["Sprawdź naszą ofertę już dziś.", "Szybka dostawa i wsparcie."],
        keywords: [],
        geoTargets: ["PL"],
        youtubeVideoIds: [],
        longHeadlines: [],
