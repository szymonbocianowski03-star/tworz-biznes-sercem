...
import { googleAdsAdapter } from "../adapters/google.adapter";
...
export async function processLaunchJob(admin: AdminClient, jobId: string): Promise<ProcessLaunchJobResult> {
  ...
  if (payload.channel.provider === "meta") {
    ...
    let gid = (draft.sync_state as Record<string, Json> | null)?.google_connection_id ?? payload.channel.googleConnectionId;
  ...
  if (payload.channel.provider === "google" && payload.channel.googleConnectionId) {
    priorIds.google_connection_id = payload.channel.googleConnectionId;
  }
