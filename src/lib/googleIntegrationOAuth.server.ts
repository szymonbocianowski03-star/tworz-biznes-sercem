...
async function listGoogleAdsAccessibleCustomers(accessToken: string): Promise<GoogleAdsCustomerAccount[]> {
...
export async function handleGoogleIntegrationOAuthCallback(
  ...
  try {
    ...
    if (svc === "ads") {
      const customerAccounts = await listGoogleAdsAccessibleCustomers(accessToken);
      const { error: upsertErr } = await supabaseAdmin.from("google_ads_connections").upsert(
        {
          ...baseRow,
          customer_accounts: customerAccounts,
          selected_customer_id: customerAccounts[0]?.id ?? null,
          status: "connected",
...
function redirectIntegrationBack(
  ...
  const key = service === "calendar" ? "gcal" : service === "ads" ? "gads" : "gmail";
