...
export function LinkedInIntegrationCard() {
  ...
  const updateField = async (patch: Partial<Connection>) => {
    if (!conn) return;
    const { error } = await supabase.from("linkedin_connections").update(patch).eq("id", conn.id);
          ...
          <div className="mt-6">
            {comingSoon ? (
              <p className="text-sm text-muted-foreground">
                Integracja LinkedIn Ads będzie dostępna <span className="font-semibold text-foreground">wkrótce</span>.
              </p>
            ) : (
              <>
                <button
