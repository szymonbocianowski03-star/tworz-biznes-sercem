...
export function LaunchPanel({
  ...
  onLoadItems,
  onCancelJob,
}: Pick<BuilderProps, "jobs" | "jobItems" | "activeJob" | "onRefreshJobs" | "onLoadItems" | "onCancelJob">) {
      ...
      <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950">
        <div className="flex flex-wrap items-center gap-2">
          <Button
            ...
            disabled
            aria-disabled
            title="Wkrótce zautomatyzowane — automatyczna publikacja jest w przygotowaniu"
          >
            Opublikuj kampanię
          </Button>
          <span className="inline-flex items-center rounded-full border border-amber-300 bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700 dark:border-amber-700/50 dark:bg-amber-950/40 dark:text-amber-400">
            Wkrótce zautomatyzowane
          </span>
        </div>
        <p className="mt-2 text-xs text-muted-foreground">
          Szkic kampanii jest gotowy i w pełni dostępny do edycji. Automatyczna publikacja na kanale pojawi się wkrótce —
          na razie przygotuj kampanię tutaj i przenieś ją do panelu reklamowego ręcznie.
        </p>
      </div>
