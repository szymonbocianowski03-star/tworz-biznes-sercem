...
export function UserIntegrationsCard() {
  ...
  const [acceptedEmailRisk, setAcceptedEmailRisk] = useState(false);
  ...
  const sendTest = async () => {
    if (!acceptedEmailRisk) {
      toast.error("Musisz zaakceptować wysyłkę na własne ryzyko.");
      return;
    }
    const { data: auth } = await supabase.auth.getUser();
  ...
  const canSendTest = Boolean(email?.gmail || email?.outlook || email?.smtp);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-foreground/10 bg-card p-5">
        ...
        <div className="mb-4 rounded-lg border border-amber-600/25 bg-amber-50/80 px-3 py-2 text-xs leading-relaxed text-amber-950 dark:bg-amber-950/30 dark:text-amber-100">
          Wysyłka działa <strong>na własne ryzyko</strong>. Przed każdą wiadomością musisz świadomie kliknąć akceptację —
          odpowiadasz za treść, odbiorców i zgodność z prawem (RODO, spam).
        </div>
      ...
      {sendConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-4 sm:items-center">
          <div className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-xl">
            <h3 className="font-display text-lg font-bold">Potwierdź wysyłkę e-mail</h3>
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              Wiadomość zostanie wysłana z Twojego połączonego konta. Działasz{" "}
              <strong className="text-foreground">na własne ryzyko</strong> — odpowiadasz za treść i odbiorcę.
              MarketingNow nie ponosi odpowiedzialności za skutki wysyłki.
            </p>
            <label className="mt-4 flex cursor-pointer items-start gap-3 rounded-lg border border-border bg-muted/30 p-3 text-sm">
              <input
                type="checkbox"
                className="mt-1"
                checked={acceptedEmailRisk}
                onChange={(e) => setAcceptedEmailRisk(e.target.checked)}
              />
              <span>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button
                disabled={!acceptedEmailRisk || sendingTest}
                onClick={() => void sendTest()}
                className="rounded-lg bg-foreground px-3 py-1.5 text-xs font-semibold text-background disabled:opacity-50"
              >
                {sendingTest ? "Wysyłanie…" : "Tak, wyślij"}
              </button>
              <button
                onClick={() => {
                  setSendConfirmOpen(false);
      ...
      <section className="rounded-xl border border-foreground/10 bg-card p-5">
        ...
        <div className="mb-4 rounded-lg border border-amber-600/25 bg-amber-50/80 px-3 py-2 text-xs leading-relaxed text-amber-950 dark:bg-amber-950/30 dark:text-amber-100">
          Zapis wydarzeń działa <strong>na własne ryzyko</strong>. W czacie agenta każde dodanie do kalendarza wymaga Twojej
