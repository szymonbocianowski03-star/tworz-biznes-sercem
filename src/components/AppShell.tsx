...
const navProducts: NavItem[] = [
  { to: "/products/library", label: "Produkty", icon: Package },
...
export function AppShell() {
  ...
  const [workspaces, setWorkspaces] = useState<string[]>(() => {
    if (typeof window === "undefined") return ["Osobiste"];
    try {
      const raw = localStorage.getItem("mn.workspaces");
      return raw ? JSON.parse(raw) : ["Osobiste"];
    } catch {
      return ["Osobiste"];
    }
  });
  const [activeWs, setActiveWs] = useState<string>(() => {
    if (typeof window === "undefined") return "Osobiste";
    return localStorage.getItem("mn.activeWs") || "Osobiste";
  });
  ...
  const activeProduct = activeProductObj?.name ?? "Nowy Produkt";
  ...
  useEffect(() => {
    localStorage.setItem("mn.workspaces", JSON.stringify(workspaces));
  }, [workspaces]);
  useEffect(() => {
    localStorage.setItem("mn.activeWs", activeWs);
  }, [activeWs]);
  useEffect(() => {
  ...
  const addWorkspace = () => {
    ...
    setWorkspaces((w) => [...w, trimmed]);
    setActiveWs(trimmed);
    setOpen(false);
  };

  const renameWorkspace = (oldName: string) => {
    ...
    setWorkspaces((ws) => ws.map((w) => (w === oldName ? trimmed : w)));
    if (activeWs === oldName) setActiveWs(trimmed);
  };
  ...
  const handleCreateProduct = ({ name, thumbnail }: { name: string; thumbnail?: string }) => {
    const p = createProduct(name, { thumbnail });
    selectProduct(p.id);
                    ...
                    {workspaces.map((w) => (
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="flex-1 text-left truncate">{w}</span>
                          {activeWs === w && <Check className="h-3.5 w-3.5 text-accent shrink-0" />}
                        </button>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
            ...
            <div className="relative" ref={productWrapRef}>
              ...
              {productOpen && (
                <div className="absolute left-0 top-full mt-1.5 w-72 rounded-md border border-foreground/10 bg-background shadow-elevated p-1.5 z-30">
                  ...
                  >
                    <Package className="h-4 w-4" /> Zarządzaj produktami
                  </Link>
                </div>
              )}
            </div>
